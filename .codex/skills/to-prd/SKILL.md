---
name: to-prd
description: 이 저장소에서 기능 요청, 정책 변경, API 변경, 앱 요구사항을 docs/history/{도메인 번호}. {한글 도메인}/{0001}-{기능명-slug}/PLAN.md 형식의 PRD 또는 구현 계획으로 정리할 때 사용한다.
---
...
- `docs/history/README.md`와 주 도메인 인덱스를 기준으로 기존 작업 이력의 흐름과 맞춘다.
...
- 문서를 실제로 생성할 때는 먼저 물리적 원본을 둘 주 도메인을 하나 선택한다.
- 기존 모든 `docs/history/{도메인 번호}. {한글 도메인}/{4자리 번호}-{기능명-slug}/`를 중첩 검색해 가장 큰 번호의 다음 4자리 전역 번호를 사용한다. 번호는 도메인별로 다시 시작하지 않는다.
- 새 문서는 `docs/history/{도메인 번호}. {한글 도메인}/{4자리 번호}-{기능명-slug}/`에 만들고 `docs/history/` 루트에는 작업 디렉터리를 만들지 않는다.
- history 디렉터리명은 `0001-feature-slug`처럼 4자리 번호, 하이픈, 공백 없는 slug 형식을 사용한다.
- 작업 생성 뒤 `docs/history/README.md`의 전체 시간순 인덱스와 주 도메인 인덱스를 갱신한다. 다른 도메인에 실제 설계 영향이 있으면 해당 도메인의 연관 문서에도 이유와 함께 추가한다.
- 새 도메인이 필요하면 기존 도메인 번호를 바꾸지 않고 다음 2자리 번호와 한글 도메인명을 사용한다.
- 시스템 구조 설계 결정은 PLAN.md에만 묻지 말고 필요하면 `docs/adr/` 후보로 분리한다.
