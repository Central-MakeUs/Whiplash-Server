 ...
 *
 * - provider: MOCK, providerUserId 패턴: "{prefix}-{runId}-{index}" 형태로 고유 식별
...
public class LoadTestMemberHelper {
    ...
    public record TestMemberInfo(Long memberId, String providerUserId, String accessToken) {}
    ...
    public List<TestMemberInfo> createTestMembers(String prefix, int count) {
        ...
        for (int i = 0; i < count; i++) {
            String providerUserId = prefix + "-" + runId + "-" + i;
            ...
            result.add(new TestMemberInfo(member.getId(), providerUserId, accessToken));
        }
